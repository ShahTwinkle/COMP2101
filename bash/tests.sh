...
test -f /etc/resolv.conf && echo "/etc/resolv.conf is regular file" || echo "/etc/resolv.conf is irregular file"

# TASK 2: Add a test to see if the /etc/resolv.conf file is a symbolic link
test -h /etc/resolv.conf && echo "/etc/resolv.conf is symbolic link" || echo "/etc/resolv.conf is not a symbolic link"

# TASK 3: Add a test to see if the /etc/resolv.conf file is a directory
test -d /etc/resolv.conf && echo "/etc/resolv.conf is directory" || echo "/etc/resolv.conf is not a directory"
...
test -w /etc/resolv.conf && echo "/etc/resolv.conf is writable" || echo "/etc/resolv.conf is writable"

# TASK 6: Add a test to see if the /etc/resolv.conf file is executable
test -x /etc/resolv.conf && echo "/etc/resolv.conf is executable" || echo "/etc/resolv.conf is inexecutable"
...
[ -rwx /tmp ] && echo "/tmp directory can be accessed" || echo "/tmp directory cannot be accessed"
...
# TASK 2:
[ $firstNumber -eq 0 $secondNumber ] && echo "The two numbers are even"
[ $firstNumber -eq 1 $secondNumber ] && echo "The two numbers are odd"
